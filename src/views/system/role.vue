...
<script>
  ...
  computed: {
    ...
    permissionList() {
      return {
        addBtn: this.vaildData(this.permission.role_add, true),
        viewBtn: this.vaildData(this.permission.role_view, true),
        delBtn: this.vaildData(this.permission.role_delete, true),
        editBtn: this.vaildData(this.permission.role_edit, true)
      };
    },
    ids() {
      let ids = [];
  ...
  methods: {
    ...
    handleRole() {
      if (this.selectionList.length == 0) {
        this.$message.warning("请先选择要设置的角色");
        return;
      }
