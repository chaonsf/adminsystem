            ...
            <el-table-column width='80' align='center' label="审批" fixed='right'>
                <template slot-scope="scope">
                    ...
                    <div v-if='scope.row.status==2'>
                        ...
                        <span class='anniu light' @click='throught(scope.row,3)'>不通过</span>
                    </div>
                    <span v-if='scope.row.status==3' class='red'>未批准</span>
                </template>
            </el-table-column>
    ...
    methods: {
        ...
        throught(item, id) {
            ...
            this.$popup({
                text: text,
                ok: () => {
                    this.toexam(item.harmlessId, id)
                }
            })
        },
        toexam(harmlessId, id) {
            let param = {
