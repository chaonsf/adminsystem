<template>
  <basic-container>
    <div class="linkage">
      ...
      <el-select v-model="shi" @change="choseCity" placeholder="选择地市">
        <el-option v-for="item in shi1" :key="item.id" :label="item.value" :value="item.id"></el-option>
      </el-select>
      <el-select v-model="qu" @change="choseBlock" placeholder="选择区县">
        <el-option v-for="item in qu1" :key="item.id" :label="item.value" :value="item.id"></el-option>
      </el-select>
      <el-button type="primary" size="small" plain icon="el-icon-refresh" @click="getRegion">确定</el-button>
    </div>
  </basic-container>
</template>
